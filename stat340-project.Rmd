...
ggplot(gap, aes(x=title, y=basesalary, color="cyan")) +
  geom_col() +
...
We previously found that there is a gender gap of base salary in STEM fields. And if we breakdown STEM fields into these 15 job titles, we can see that male workers still have a higher median base salary in most jobs. The gender gap for Management Consultant is the most extreme. 

***

```{r, inlcude=FALSE, warning=FALSE, message=FALSE}
filted_data %>%
  group_by(title, gender) %>% 
  summarise(n=n()) %>% 
  filter(title == "Mechanical Engineer")
```

***

As we saw above, Mechanical Engineer is the only job which women has a higher median base salary than men does. However, we only have 39 data entries for female Mechanical Engineer. Therefore, the median base salary for female Mechanical Engineer can change dramatically if we have more data of that. 
